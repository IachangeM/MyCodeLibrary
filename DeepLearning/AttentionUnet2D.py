...
class UnetGridGatingSignal2(nn.Module):
    def __init__(self, in_size, out_size, kernel_size=(1,1,1), is_batchnorm=True):
        ...
        if is_batchnorm:
            self.conv1 = nn.Sequential(nn.Conv2d(in_size, out_size, kernel_size, stride=(1, 1), padding=(0, 0)),
                                       nn.BatchNorm2d(out_size),
                                       nn.ReLU(inplace=True),
                                       )
        else:
...
class AttentionBlock2D(_GridAttentionBlockND):
    def __init__(self, in_channels, gating_channels, inter_channels=None, mode='concatenation',
                 sub_sample_factor=(2,2,2)):
        super(AttentionBlock2D, self).__init__(in_channels,
...
class attention_unet_2D(nn.Module):
    # TODO: attention_dsample=(2,2)/(1,1)都可以 这个参数的作用是什么?
    def __init__(self, feature_scale=4, n_classes=21, is_deconv=True, in_channels=3,
        ...
        # attention blocks
        self.attentionblock2 = AttentionBlock2D(in_channels=filters[1], gating_channels=filters[3],
                                                inter_channels=filters[1], sub_sample_factor=attention_dsample, mode=nonlocal_mode)
        self.attentionblock3 = AttentionBlock2D(in_channels=filters[2], gating_channels=filters[3],
                                                inter_channels=filters[2], sub_sample_factor=attention_dsample, mode=nonlocal_mode)
        self.attentionblock4 = AttentionBlock2D(in_channels=filters[3], gating_channels=filters[3],
...
if __name__ == '__main__':
    ...
    attunet = attention_unet_2D(feature_scale=1, n_classes=1, in_channels=3)
    img_np =np.random.random((1,3,512,512))
    inp = torch.from_numpy(img_np)
    inp = inp.float()

    print(" input size:", inp.size())
    otp = attunet(inp)
    print("output size:", otp.size())
